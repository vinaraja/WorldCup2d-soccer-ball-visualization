package 
{
	...
	public class Main extends GestureWorks 
	{
		...
		private function sphereButtonHandler(event:StateEvent):void
		{
			...
			for each (var button:Button in sphereButtons)
			{
				button.removeEventListener(StateEvent.CHANGE,sphereButtonHandler)
			}
			
			
			// restart idle timer when someone presses one of the buttons
			_idleTimer.reset();
			_idleTimer.start();
			...
			if (first == 1)
			{
			var image1:Image = CMLObjectList.instance.getId("1");
			image1.alpha = 0;
			}
			first = first + 1;
			var id:String = event.target.id;
			var image:Image = CMLObjectList.instance.getId(String(oldid));
			image.alpha = 0;
			var ba:ByteArray = new ByteArray();
			if (id == "21")
			{
			...
			auxDisplay = CMLParser.cmlData.attribute("auxDisplay");
			trace (auxDisplay);
            var image3:Image = CMLObjectList.instance.getId(newid);
			image3.alpha = 1;
			if (auxDisplay == "sos")
			{
			...
			auxDisplay = CMLParser.cmlData.attribute("auxDisplay");
			trace (auxDisplay);
			 var image:Image = CMLObjectList.instance.getId(newid);
			image.alpha = 1;
      ...
      myDelay.addEventListener(TimerEvent.TIMER, showMessage);
        ...
        function showMessage(e:TimerEvent):void {
           ...
           //trace ("delay");
			...
			e.target.stop();
			//trace("brooo")
		    //e.target.removeEventListner(TimerEvent.TIMER, showMessage)
			//myDelay.removeEventListener();
			//trace("bulllll")
